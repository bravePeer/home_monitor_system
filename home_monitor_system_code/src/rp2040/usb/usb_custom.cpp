...
static usb_device_configuration dev_config = {
...
/**
 * @brief Given an endpoint address, return the usb_endpoint_configuration of that endpoint. Returns nullptr
...
uint8_t usb_prepare_string_descriptor(const unsigned char *str) {
    ...
    uint8_t bLength = 2 + static_cast<uint8_t>(strlen((const char *)str) * 2);
 ...
 * @return false
 */
static inline bool ep_is_tx(const usb_endpoint_configuration *ep) {
...
void usb_handle_string_descriptor(volatile struct usb_setup_packet *pkt) {
    ...
    if (i == 0) {
        ...
        len = usb_prepare_string_descriptor((const unsigned char *) dev_config.descriptor_strings[i - 1]);
    }
 ...
 * @param pkt, the setup packet from the host.
 */
void usb_set_device_configuration(__unused volatile usb_setup_packet *pkt) {
...
static void usb_handle_ep_buff_done(struct usb_endpoint_configuration *ep) {
    ...
    ep->handler(const_cast<uint8_t *>(ep->data_buffer), len);
}
...
void ep1_out_handler(uint8_t *buf, uint16_t len) {
    uint8_t txDataPtr[64]{0};
    uint16_t txLen = 0;

    // if(static_cast<uint8_t>(len) != buf[1])
    // {
    //     txDataPtr[0] = getUsbResponseCommand(usbCommand, UsbErrorCode::LenghtError);
    //     txDataPtr[1] = 0;
    //     txLen = 2;
    //     goto endOfHandler;
    // }

    txDataPtr[0] = processUsbCommands(buf, len, &txDataPtr[2], txLen);
    txLen += 2;
    txDataPtr[1] = static_cast<uint8_t>(txLen);


    usb_start_transfer(usb_get_endpoint_configuration(EP2_IN_ADDR), txDataPtr, txLen);
}
...
void usb_device_init() {
    ...
    irq_set_exclusive_handler(USBCTRL_IRQ, isr_usbctrl_test);
