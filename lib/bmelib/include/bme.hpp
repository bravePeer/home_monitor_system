...
namespace bme
{
    ...
    struct BmeSelector<BmeType::BME280>
    {
        ...
        // template <size_t N>
        static void readAllData(uint8_t *data)
        ...
        {
            static_assert(N >= 16, "Buffer size must be at least 6 bytes to read pressure and temperature data");
            transmitSpiBme280(data, data, commandReadBytes(bme280::MemoryMap::CalibH2LSB), 16);
        }
    
        static void startForcedMeasurement()
        {
            uint8_t data = 0;
            transmitSpiBme280(&data, &data, commandReadBytes(bme280::MemoryMap::CtrlMeas), 1);
            data = (data & (~0x03)) | static_cast<uint8_t>(bme::ControlMode::Forced);
            transmitSpiBme280(&data, &data, commandWriteBytes(bme280::MemoryMap::CtrlMeas), 1);
        }
    };
